...
1. **Ortho 内存对齐** ✅
   - 添加了 `orth_layer_alloc_ortho()` 函数 (`include/ortho.h:91-95`, `src/ortho.c:113-170`)
   - 实现了 128-byte 对齐的内存分配
   - 更新了 `orth_layer_free()` 使用对齐释放
...
4. **测试代码更新** ✅
   - 更新了 `tests/test_cpu_forward.c` 使用对齐分配函数
...
1. **性能验证**：
   - 运行 `tests/benchmark_null_test.c` 验证开销 < 1%
   - 在实际硬件上测试性能

2. **可选优化**：
   - 考虑数据类型一致性（`half` vs `float`）
   - 完善实验 3 (Dual-DP) 的实现
